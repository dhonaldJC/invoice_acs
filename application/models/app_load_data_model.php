...
class app_load_data_model extends CI_Model {
	...
	/**
	 * @author 		: Dhonald Johannes C.H. Sihotang
	 * @keterangan 	: Model untuk Invoice
	 **/
	...
	public function indexs_data_service($limit,$offset)
	{
		$hasil = "";
		$hasil .= '
			<table class="table table-striped table-bordered bootstrap-datatable datatable">
		...
		$where['nama_service']  = $this->session->userdata("key"); 
		$tot_hal = $this->db->like($where)->get("dlmbg_jenis_service");
		$config['base_url'] = base_url() . 'dashboard/jenis_service/index/';
		$config['total_rows'] = $tot_hal->num_rows();
		$config['per_page'] = $limit;
		$config['uri_segment'] = 4;
		$this->pagination->initialize($config);
		$get = $this->db->like($where)->order_by("kode_jenis_service","DESC")->get("dlmbg_jenis_service",$limit,$offset);
		$i = $offset+1;
		foreach($get->result() as $g)
		{
			$hasil .= ' <tbody>
				<tr>
					<td>'.$i.'</td>
					<td>'.$g->jenis_service.'</td>
					<td class="center">
						<a class="btn btn-info" href="'.base_url().'dashboard/jenis_service/edit/'.$g->kode_jenis_service.'">
							<i class="halflings-icon edit halflings-icon"></i>  
						</a>
						<a class="btn btn-danger" href="'.base_url().'dashboard/jenis_service/hapus/'.$g->kode_jenis_service.'" onClick=\'return confirm("Anda yakin?");\'>
							<i class="halflings-icon trash halflings-icon"></i> 
						</a>
					</td>
				</tr>';
	...
	public function indexs_data_jenis_service($limit,$offset)
	{
		$hasil = "";
		$hasil .= '
			<table class="table table-striped table-bordered bootstrap-datatable datatable">
		...
		$config['uri_segment'] = 4;
		$this->pagination->initialize($config);
		$get = $this->db->query("select * from dlmbg_jenis_service a left join dlmbg_jenis_satuan b on a.id_jenis_satuan=b.id_jenis_satuan where nama_service like '%".$where['nama_service']."%' limit ".$offset.",".$limit."");
		$i = $offset+1;
		foreach($get->result() as $g)
		{
			$hasil .= ' <tbody>
		...
		foreach($get->result() as $g)
		{
			$hasil .= ' <tbody>
				<tr>
					...
					<td>'.$g->nama_pelanggan.'</td>
					<td>'.$g->alamat_pelanggan.'</td>';
					$get_detail = $this->db->select("*")->join("dlmbg_jenis_service","dlmbg_jenis_service.kode_jenis_service=dlmbg_pemesanan_detail.kode_jenis_service")->get_where("dlmbg_pemesanan_detail",array("kode_pemesanan"=>$g->kode_pemesanan));
					$hasil .= "<th>";

					foreach($get_detail->result() as $gd)
					{
						$hasil .= "<li>".$gd->nama_service."</li>";
					}
