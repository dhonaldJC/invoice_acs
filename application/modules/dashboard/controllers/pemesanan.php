...
class pemesanan extends CI_Controller {
	...
	function tambah()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$d['jenis_service'] = $this->db->get("dlmbg_jenis_service");
	...
	function tambah_barang_pesanan()
	{
		...
		{
			$data = array(
				'id'      => $this->input->post('kode_jenis_service'),
				'qty'     => $this->input->post('jumlah'),
				'price'   => $this->input->post('harga'),
				'name'    => $this->input->post('nama_service'),
	...
	function ambil_data_bahan()
	{
		if($this->session->userdata("logged_in")!="")
		{
			$kd = $_GET["kode_jenis_service"];
			$q = $this->db->query("select * from dlmbg_jenis_service a left join dlmbg_jenis_satuan b on a.id_jenis_satuan=b.id_jenis_satuan where a.kode_jenis_service='".$kd."'");
			foreach($q->result() as $d)
			{
			?>
			<table cellpadding="0" cellspacing="0" border="0">
			<tr><td width="160">Kode Service</td><td width="20">:</td><td><input type="text" value="<?php echo $d->kode_jenis_service; ?>" class="input-read-only"
			 style="width:350px;" name="kode_jenis_service" readonly="readonly" /></td></tr>
			<tr><td>Nama Service</td><td>:</td><td><input type="text" value="<?php echo $d->nama_service; ?>" class="input-read-only" style="width:350px;" name=
			"nama_service" readonly="readonly" /></td></tr>
	...
	function edit($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$get_detail = $this->db->query("select a.kode_jenis_service, a.jumlah, b.nama_service, b.harga, b.satuan from dlmbg_pemesanan_detail a left join 
			(select x.nama_service, y.satuan, x.kode_jenis_service, x.harga from dlmbg_jenis_service x left join dlmbg_jenis_satuan y on x.id_jenis_satuan=y.id_jenis_satuan) 
			b on a.kode_jenis_service=b.kode_jenis_service where a.kode_pemesanan='".$id_param."'");
			
			$in_cart = array();
			foreach($get_detail->result() as $dpd)
			{
				$in_cart[] = array(
				'id'      => $dpd->kode_jenis_service,
				'qty'     => $dpd->jumlah,
				'price'   => $dpd->harga,
				'name'    => $dpd->nama_service,
	...
	function cetak($id_param)
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			$get_detail = $this->db->query("select a.kode_jenis_service, a.jumlah, b.harga, b.nama_service, b.satuan from dlmbg_pemesanan_detail a left join 
			(select x.nama_service, y.satuan, x.kode_jenis_service, x.harga from dlmbg_jenis_service x left join dlmbg_jenis_satuan y on x.id_jenis_satuan=y.id_jenis_satuan) 
			b on a.kode_jenis_service=b.kode_jenis_service where a.kode_pemesanan='".$id_param."'");
	...
	function simpan()
	{
		if($this->session->userdata("logged_in")!="")
		{
			...
			foreach($this->cart->contents() as $items)
			{
				...
				$d_detail['kode_jenis_service'] = $items['id'];
