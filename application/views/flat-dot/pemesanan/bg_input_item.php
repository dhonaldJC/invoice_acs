...
<body>
	...
	<div class="well">
	<h1>Data - Master Jenis Service</h1>
		<div id="body">
		<?php $atr = array('name' => 'frm', 'id' => 'frm'); echo form_open('dashboard/pemesanan/tambah_barang_pesanan',$atr); ?>
			<table width="100%" cellpadding="3" cellspacing="0">
				<tr><td width="130">Nama Service</td><td>:</td><td>
				<select data-placeholder="Cari nama service..." class="chzn-select" style="width:400px;" tabindex="2" name="kode_jenis_service" id="kode_jenis_service">
          		<option value=""></option> 
					<?php
						foreach($jenis_service->result_array() as $db)
						{
					?>
						<option value="<?php echo $db['kode_jenis_service']; ?>"><?php echo $db['nama_service']; ?></option>
					<?php
						}
					?>
				</select>
			...
			<script type="text/javascript"> $(".chzn-select").chosen().change(function(){ 
						var kode_jenis_service = $("#kode_jenis_service").val(); 
						$.ajax({ 
						url: "<?php echo base_url(); ?>dashboard/pemesanan/ambil_data_bahan", 
						data: "kode_jenis_service="+kode_jenis_service, 
						cache: false, 
